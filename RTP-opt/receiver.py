...
def handle_data_packet(
    ...
    """Handle DATA packet with selective ACK semantics."""
    if header.seq_num < next_expected:
        sock.sendto(build_ack(header.seq_num), sender_addr)
    elif header.seq_num == next_expected:
        flush_to_stdout(payload)
        sock.sendto(build_ack(header.seq_num), sender_addr)
        next_expected += 1
...
def receiver(receiver_ip, receiver_port, window_size):
    """Receive data over UDP using selective-ACK semantics."""
    if window_size <= 0:
